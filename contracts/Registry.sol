...
contract Registry{
    ...
    struct LandDetails{
        address[] previousOwners; // Array to store previous owners
        address owner;
        address admin;
        uint256 propertyId;
        uint surveyNumber;
    ...
    function AcceptRequest(uint _index, uint _reqNo) external{
        ...
        // Add the current owner to the history
        landDetalsMap[_state][_district][_city][_surveyNo].previousOwners.push(msg.sender);
    ...
    // Function to get the history of owners for a land
    function getOwnerHistory(string memory _state, string memory _district, string memory _city, uint _surveyNo) external view returns (address[] memory) {
        return landDetalsMap[_state][_district][_city][_surveyNo].previousOwners;
    }

//     function getLandDetailsBySurveyNo(uint _surveyNo) external view returns(address, uint256, uint, uint, uint) {
//     // Retrieve land details using survey number
//        address owner = landDetalsMap[_state][_district][_city][_surveyNo].owner;
//         uint256 propertyid = landDetalsMap[_state][_district][_city][_surveyNo].propertyId;
//         uint indx = landDetalsMap[_state][_district][_city][_surveyNo].index;
//         uint mv = landDetalsMap[_state][_district][_city][_surveyNo].marketValue;
...
// }
    ...
    // function getAllRegisteredLandDetails() external view returns (uint[] memory, string[] memory, string[] memory) {
    //     // Initialize arrays to store survey numbers and cities
    //     uint[] memory surveyNumbers = new uint[](totalAdmins);
    //     string[] memory cities = new string[](totalAdmins);
    //     string[] memory states = new string[](totalAdmins);
        
    //     uint landIndex = 0;

    //     // Iterate through all admins
    //     for (uint i = 0; i < totalAdmins; i++) {
    //         address adminAddr = admins[adminKeys[i]].adminAddress;
    //         // Iterate through all land details in the admin's jurisdiction
    //         for (uint j = 0; j < userProfile[adminAddr].totalIndices; j++) {
    //             string memory state = ownerMapsProperty[adminAddr][j].state;
    //             string memory district = ownerMapsProperty[adminAddr][j].district;
    //             string memory city = ownerMapsProperty[adminAddr][j].city;
    //             uint surveyNo = ownerMapsProperty[adminAddr][j].surveyNumber;
                
    //             // Store survey number and city
    //             surveyNumbers[landIndex] = surveyNo;
    //             cities[landIndex] = city;
    //             states[landIndex] = state;
    //             landIndex++;
    //         }
    //     }
    //     return (surveyNumbers, cities, states);
    // }
